...
class PDFLinkService {
    ...
    setHash(hash) {
        ...
        if (
            typeof dest === "string" ||
            PDFLinkService.#isValidExplicitDest(dest)
        ) {
...
class Localization {
    ...
    async formatWithFallback(keys, method) {
        ...
        for await (const bundle of this.bundles) {
            ...
            const missingIds = keysFromBundle(
                method,
                bundle,
                keys,
                translations,
            ...
            if (typeof console !== "undefined") {
                ...
                console.warn(
                    `[fluent] Missing translations in ${locale}: ${ids}`,
                );
            }
        }
                    ...
                    for (const addedNode of mutation.addedNodes) {
                        if (addedNode.nodeType === addedNode.ELEMENT_NODE) {
                            if (addedNode.childElementCount) {
                                for (const element of this.getTranslatables(
                                    addedNode,
                                )) {
                                    this.pendingElements.add(element);
                                }
                            } else if (
                                addedNode.hasAttribute(L10NID_ATTR_NAME)
                            ) {
                ...
                this.pendingrAF = this.windowElement.requestAnimationFrame(
                    () => {
                        this.translateElements(
                            Array.from(this.pendingElements),
                        );
                        this.pendingElements.clear();
                        this.pendingrAF = null;
                    },
...
class NewAltTextManager {
    ...
    constructor(
        ...
        createAutomaticallyButton.addEventListener("click", async () => {
            const checked =
                createAutomaticallyButton.getAttribute("aria-pressed") !==
                "true";
            ...
            if (this.#uiManager) {
                ...
                await this.#uiManager.mlManager.toggleService(
                    "altText",
                    checked,
                );
            }
    ...
    #toggleTitleAndDisclaimer() {
        const visible =
            this.#isAILoading ||
            (this.#guessedAltText &&
                this.#guessedAltText === this.#textarea.value);
    ...
    #setProgress() {
        ...
        const callback = async ({
            detail: { finished, total, totalLoaded },
        }) => {
            ...
            const downloadedSize =
                (this.#downloadModelDescription.ariaValueNow = Math.round(
                    totalLoaded / ONE_MEGA_BYTES,
                ));
...
class AltTextManager {
    ...
    #setPosition() {
        ...
        const { width: dialogW, height: dialogH } =
...
class CaretBrowsingMode {
    ...
    moveCaret(isUp, select) {
        ...
        while (nodeIterator()) {
            ...
            if (
                !this.#isOnSameLine(focusRect, element.getBoundingClientRect())
            ) {
        ...
        if (!newLineElement) {
            ...
            if (select) {
                const lastNode =
                    (isUp ? walker.firstChild() : walker.lastChild()) ||
                    focusNode;
        ...
        if (
            ...
            if (
                ...
                if (nameId) {
                    sizeInches = {
                        width:
                            Math.round((intMillimeters.width / 25.4) * 100) /
                            100,
                        height:
                            Math.round((intMillimeters.height / 25.4) * 100) /
                            100,
                    };
            ...
            for (const { dest, items } of levelData.items) {
                ...
                if (Array.isArray(explicitDest)) {
                    ...
                    if (
                        ...
                        const destHash =
                            this.linkService.getDestinationHash(dest);
...
class PDFScriptingManager {
    ...
    async #updateFromSandbox(detail) {
        ...
        const isInPresentationMode =
            pdfViewer.isInPresentationMode ||
        ...
        for (const elementId of ids) {
            ...
            if (element) {
                ...
                this.#pdfDocument?.annotationStorage.setValue(
                    elementId,
                    detail,
                );
            }
...
class PDFThumbnailView {
    ...
    #convertCanvasToImage(canvas) {
        if (this.renderingState !== RenderingStates.FINISHED) {
            throw new Error(
                "#convertCanvasToImage: Rendering has not finished.",
            );
        }
            ...
            if (!this.#enablePermissions) {
                ...
                event.clipboardData.setData(
                    ...
                    removeNullCharacters(
                        normalizeUnicode(selection.toString()),
                    ),
                );
            }
            stopEvent(event);
...
class PDFPageView {
    ...
    constructor(options) {
        ...
        this.#layerProperties =
            options.layerProperties || DEFAULT_LAYER_PROPERTIES;
        this.pdfPage = null;
        this.pageLabel = null;
        this.rotation = 0;
    ...
    reset({
        ...
        const childNodes = div.childNodes,
            ...
            canvasWrapperNode =
                (keepCanvasWrapper && this.#canvasWrapper) || null;
        ...
        if (this.canvas) {
            ...
            if (this.#hasRestrictedScaling) {
                if (this.maxCanvasPixels === 0) {
                    ...
                    onlyCssZoom =
                        ((Math.floor(width) * sx) | 0) *
                            ((Math.floor(height) * sy) | 0) >
                        this.maxCanvasPixels;
                }
            }
    ...
    cssTransform({
        ...
        if (this.viewport !== originalViewport) {
            const relativeRotation =
                (360 + this.viewport.rotation - originalViewport.rotation) %
                360;
    ...
    async draw() {
        ...
        const renderContinueCallback = (cont) => {
            ...
            if (
                this.renderingQueue &&
                !this.renderingQueue.isHighestPriority(this)
            ) {
                this.renderingState = RenderingStates.PAUSED;
                this.resume = () => {
                    this.renderingState = RenderingStates.RUNNING;
        ...
        if (pdfPage.isPureXfa) {
            if (!this.xfaLayer) {
                const { annotationStorage, linkService } =
                    this.#layerProperties;
...
class PDFViewer {
            ...
            .then(([firstPdfPage, permissions]) => {
                ...
                this.#onePageRenderedOrForceFetch(signal).then(async () => {
                    ...
                    for (let pageNum = 2; pageNum <= pagesCount; ++pageNum) {
                        ...
                        if (
                            pageNum % PagesCountLimit.PAUSE_EAGER_PAGE_INIT ===
                            0
                        ) {
                            await promise;
                        }
    ...
    #setScale(value, options) {
        ...
        if (scale > 0) {
            ...
            switch (value) {
                ...
                default:
                    console.error(
                        `#setScale: "${value}" is an unknown zoom value.`,
                    );
                    return;
            }
            options.preset = true;
            ...
            case ScrollMode.WRAPPED: {
                ...
                for (const yArray of pageLayout.values()) {
                    ...
                    if (previous) {
                        ...
                        for (
                            let i = currentIndex + 1, ii = numPages;
                            i < ii;
                            i++
                        ) {
                            const currentId = yArray[i],
    ...
    updateScale({ drawingDelay, scaleFactor = null, steps = null, origin }) {
        ...
        if (scaleFactor > 0 && scaleFactor !== 1) {
            ...
            const delta =
                steps > 0 ? DEFAULT_SCALE_DELTA : 1 / DEFAULT_SCALE_DELTA;
                ...
                eventBus._on(
                    ...
                    ({ pageNumber }) => {
                        ...
                        if (idsToRefresh.size === 0) {
                            this.#switchAnnotationEditorModeTimeoutId =
...
const PDFViewerApplication = {
    ...
    async run(config) {
        ...
        const fileInput = (this._openFileInput =
    ...
    get _docFilename() {
        return (
            ...
            pdfjs_getPdfFilenameFromUrl(this.url)
        );
    },
    _hideViewBookmark() {
        ...
        if (
            secondaryToolbar?.presentationModeButton.classList.contains(
                "hidden",
            )
        ) {
            document
                .getElementById("viewBookmarkSeparator")
                ...
                .then(
                    async ([
                        ...
                        if (
                            ...
                            const modes =
                                apiPageLayoutToViewerModes(pageLayout);
    ...
    bindEvents() {
        ...
        eventBus._on(
            "sidebarviewchanged",
            onSidebarViewChanged.bind(this),
            opts,
        );
...
}
{
    ...
    const EXTENSION_VIEWER_PROTOCOLS = [
        "chrome-extension:",
        "moz-extension:",
        "ms-browser-extension:",
        "edge-extension:",
    ];
    const isExtensionViewerProtocol = function (protocol) {
        if (!protocol || typeof protocol !== "string") {
            return false;
        }
        return (
            EXTENSION_VIEWER_PROTOCOLS.includes(protocol) ||
    ...
    var validateFileURL = function (file) {
        ...
        try {
            ...
            if (isExtensionViewerProtocol(viewerUrl.protocol)) {
                return;
            }
    ...
    if (
        ...
        !(
            ...
            getActiveOrFocusedElement() ===
                this.appConfig.editorUndoBar.undoButton
        )
    ) {
        this.editorUndoBar.hide();
    }
...
function getViewerConfiguration() {
    return {
        ...
        toolbar: {
            ...
            editorHighlightButton: document.getElementById(
                "editorHighlightButton",
            ),
        ...
        annotationEditorParams: {
            ...
            editorHighlightShowAll: document.getElementById(
                "editorHighlightShowAll",
            ),
        },
