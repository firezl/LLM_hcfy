...
(function (global) {
    function install(app) {
        ...
        function ensurePdfPrompt() {
            if (
                state.pdfPromptEl &&
                document.body.contains(state.pdfPromptEl)
            ) {
        ...
        function schedulePdfPromptAutoClose() {
            ...
            state.pdfPromptAutoCloseTimer = window.setTimeout(() => {
                if (
                    ...
                    state.pdfPromptState.pdfUrl !== snapshot.pdfUrl
                ) {
                    return;
                }
        ...
        async function checkPdfUrlAndUpdatePrompt(pdfUrl) {
            try {
                ...
                if (
                    !state.pdfPromptState ||
                    state.pdfPromptState.pdfUrl !== pdfUrl
                ) {
                    return;
                }
        ...
        function registerPdfRuntimeListener() {
            ...
            chrome.runtime.onMessage.addListener((message) => {
                ...
                if (
                    type ===
                    (app.MESSAGE_TYPES.PDF_PROMPT_OFFER || "PDF_PROMPT_OFFER")
                ) {
                    showPdfPrompt({
                        source: "background",
                ...
                if (
                    type ===
                    (app.MESSAGE_TYPES.PDF_PROMPT_VERDICT ||
                        "PDF_PROMPT_VERDICT")
                ) {
                    const promptId = String(message?.promptId || "");
                    if (
                        !state.pdfPromptState ||
                        state.pdfPromptState.promptId !== promptId
                    ) {
                        return false;
                    }
